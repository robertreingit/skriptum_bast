...
Anhand der Standardabweichung kann die Verteilung in Bereiche eingeteilt werden. Und zwar in die Bereiche $[\mu-\sigma,\mu+\sigma]$, $[\mu-2\cdot\sigma,\mu+2\cdot\sigma]$ und $[\mu-3\cdot\sigma,\mu+3\cdot\sigma]$. D.h. es wird jeweils ausgehend vom Mittelwert $\mu$ ein Bereich oberhalb und unterhalb mittels Multiplikation mit der Standardabweichung $\sigma$ ausgezeichnet (siehe @fig-ztest-dnorm-02).
...
Diese Bereiche haben jeweils ein bestimmte Wahrscheinlichkeit (siehe @tbl-ztest-dnorm).
...
Die Standardnormalverteilung hat auch eine zentrale Rolle in der theoretischen Statistik, so dass sie ein eigenes Simbol $\Phi$ spendiert bekommen hat. In @fig-ztest-stdnorm-01 ist die Dichtekurve der Standardnormalverteilung abgetragen.
...
Sieht nicht besonders spektakulär aus, ist halt auch nur eine Normalverteilung, allerdings nun mit $\mu = 0$ und $\sigma=1$. Bezogen auf die Bereiche in @tbl-ztest-dnorm bedeutet dies nun aber, dass etwa $68\%$ der Werte in den Bereich $[-1,1]$, $95\%$ in den Bereich $[-2,2]$ und $99\%$ der Werte in den Bereich $[-3,3]$. Richtig nüztlich wird, dies nun wenn die z-Transformation angewendet wird.
